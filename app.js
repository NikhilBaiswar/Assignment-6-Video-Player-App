$(document).ready(function () {
  ...
  function updateVideoData(id) {
    $("#video-player").attr(
      "src",
      "https://player.vimeo.com/video/" + videoPlaySectionData[id - 1].vimeoId
    );
    $("#views-count").html(videoPlaySectionData[id - 1].views);
    $("#video-title").html(videoPlaySectionData[id - 1].title);
    $("#video-description").html(videoPlaySectionData[id - 1].description);
  ...
  for (i = 0; i < playlistData.length; i++) {
    var card = $("<div>").addClass("playlist-card").attr("id", playlistData[i].id);
    if (i == 0) {
      card.addClass("active-card");
    }
    var thumbnail = $("<img>").addClass("thumbnail").attr("src", playlistData[i].thumbnail);
    var title = $("<h3>").html(playlistData[i].title);
    card.append(thumbnail, title);
    $("#playlist-wrapper").append(card);
    card.click(function () {
